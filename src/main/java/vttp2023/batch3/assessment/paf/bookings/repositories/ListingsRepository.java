...
public List<Document> getAccomResult(String country, int accommodates, double price) {
	...
	Query query = Query.query(c).limit(10).with(Sort.by(Sort.Direction.DESC,"price"));
	query.fields().include("_id", "description", "address", "images.picture_url", "price", "amenities");
	...
	public ListingDetails checkIfIdExists(String _id) {
		Criteria c = Criteria.where("_id").is(_id);
		Query query = Query.query(c);

		return template.findOne(query, ListingDetails.class, _id);
	}
